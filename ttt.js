...
directionalLight.position.set(1, 1, 1); // 假设光源位于场景的上方
directionalLight.castShadow = true;

const size = 50 // 选择一个足够大的数值以覆盖物体及其周围区域
directionalLight.shadow.camera.left = -size;
directionalLight.shadow.camera.right = size;
directionalLight.shadow.camera.top = size;
directionalLight.shadow.camera.bottom = -size;

directionalLight.shadow.camera.near =1; // 根据场景大小调整
directionalLight.shadow.camera.far = 50; // 确保far值足够大以覆盖物体到光源的距离

directionalLight.shadow.mapSize.width = 512; // 提高阴影质量
directionalLight.shadow.mapSize.height = 512; // 提高阴影质量
...
function init() {
    ...
    // scene.add(plane);
    loader.load('112.obj', function (object) {
        ...
        plane.geometry = new THREE.PlaneGeometry(width*8, depth*8);

        // 调整平面位置以匹配OBJ模型底部
        plane.position.x = bbox.min.x + width / 2;
        plane.position.z = bbox.min.z + depth / 2;
        plane.position.y = bbox.min.y ; // 根据实际需要可能需要调整
...
function updateLightAngle(angle) {
    ...
    directionalLight.position.z =angle
